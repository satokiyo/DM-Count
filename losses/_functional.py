...
def noise_robust_dice(
    output: torch.Tensor, target: torch.Tensor, smooth: float = 0.0, eps: float = 1e-7, dims=None, gamma: float = 1.5,
) -> torch.Tensor:
    assert output.size() == target.size()
    """
    gamma must be in range (1 - 2)
    if gamma=2 -> dice
    if gamma=1 -> weighted mae
    """
    if dims is not None:
        tmp0 = torch.sum(torch.pow(torch.abs(target - output), gamma), dim=dims)
        tmp1 = torch.sum(output, dim=dims)
        tmp2 = torch.sum(target, dim=dims)

    else:
        tmp0 = torch.sum(torch.pow(torch.abs(target - output), gamma))
        tmp1 = torch.sum(output)
        tmp2 = torch.sum(target)

    nr_dice_loss = tmp0 / (tmp1 + tmp2)
