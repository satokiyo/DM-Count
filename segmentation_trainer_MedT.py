...
class SegTrainer_MedT(object):
    ...
    def setup(self):
        ...
        # Model
        from models.MedT.lib.models.axialnet import MedT
        self.model = MedT(img_size=args.crop_size, imgchan=3)
...
# ADD
                ...
                tmp2 = masks[0].detach().cpu().numpy()
                tmp = outputs[0].detach().cpu().numpy()
                tmp[tmp>=0.5] = 1
                tmp[tmp<0.5] = 0
                tmp2[tmp2>0] = 1
                tmp2[tmp2<=0] = 0
                ...
                self.vlog.image(imgs=[yHaT[0,1,:,:]],
                          ...
                          window_ids=[2])
                self.vlog.image(imgs=[yval[0,:,:]],
                          titles=['(Training) Image yval'],
                          window_ids=[3])
# TO ADD
